...
const STATUS_OPTIONS = [
  { value: "DITERIMA",            label: "Diterima",                    color: "#3b82f6", icon: "bi-box-arrow-in-down" },
  { value: "MENUNGGU_ANTRIAN",    label: "Menunggu Antrian",            color: "#f59e0b", icon: "bi-hourglass-split" },
  { value: "PEMESANAN_SPAREPART", label: "Pemesanan Sparepart",         color: "#ec4899", icon: "bi-cart3" },
  { value: "PROSES_PENGERJAAN",   label: "Proses Pengerjaan & Testing", color: "#8b5cf6", icon: "bi-gear-wide-connected" },
  { value: "SELESAI",             label: "Selesai Service",             color: "#10b981", icon: "bi-check-circle" },
];
...
function InfoRow({ icon, label, value }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem", padding: "0.7rem 0", borderBottom: "1px solid #111c2d" }}>
      <i className={`bi ${icon}`} style={{ color: "#38bdf8", fontSize: "0.9rem", marginTop: 2, minWidth: 16 }}></i>
      <span style={{ minWidth: 130, fontSize: "0.76rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: "#475569", paddingTop: 2 }}>
        {label}
      </span>
...
function SectionDivider({ icon, label }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", margin: "1.75rem 0 1.25rem" }}>
      ...
      <span style={{ fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#475569", whiteSpace: "nowrap" }}>
    ...
    <>
      <style>{`
        ...
        .status-select {
          ...
          border-radius: 10px; color: #f1f5f9; padding: 0.65rem 1rem;
        ...
        .status-select:focus { border-color: #38bdf8; box-shadow: 0 0 0 3px rgba(56,189,248,0.1); }
        .status-select option { background: #0d1b2e; color: #f1f5f9; }
        .text-input {
          width: 100%; background: #0a1628; border: 1.5px solid #1e293b;
          border-radius: 10px; color: #f1f5f9; padding: 0.65rem 1rem;
          font-size: 0.9rem; font-family: 'DM Sans', sans-serif;
        ...
        textarea.text-input { resize: vertical; min-height: 110px; line-height: 1.6; }
        .field-label {
        ...
        .workflow-wrap { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 1rem; }
        .workflow-chip {
          display: inline-flex; align-items: center; gap: 4px;
          padding: 3px 10px; border-radius: 999px; font-size: 0.72rem;
          font-weight: 600; cursor: pointer; border: 1px solid transparent;
          transition: all 0.15s; letter-spacing: 0.03em; background: transparent;
        }
        @keyframes shimmer {
        ...
        <div style={{ maxWidth: "700px", margin: "0 auto", padding: "2.5rem 1.25rem 5rem" }}>
          ...
          <div className="fade-up" style={{ marginBottom: "2rem" }}>
            ...
            <p style={{ color: "#64748b", fontSize: "0.875rem", margin: 0, paddingLeft: 50 }}>
              Perbarui status servis — notifikasi email otomatis dikirim ke pelanggan jika ada.
            </p>
          </div>

          {fetchErr && (
            <div className="fade-up d-flex align-items-start gap-3 p-4 mb-4"
              ...
              <Link href="/admin" className="btn-back ms-auto" style={{ flexShrink: 0 }}>Kembali</Link>
            </div>
          )}
          ...
          {!fetchErr && (
            <div className="fade-up" style={{
              ...
              borderRadius: "16px", overflow: "hidden", animationDelay: "0.06s",
            }}>
              {/* Info Unit */}
              <div style={{ padding: "1.5rem 1.75rem 0" }}>
                <SectionDivider icon="bi-box-seam" label="Informasi Barang" />
                ...
                {fetching ? (
                  ...
                  <div style={{ paddingBottom: "0.5rem" }}>
                    <div style={{
                      ...
                      padding: "0.75rem 1rem", background: "#071224",
                      ...
                      <div>
                        ...
                        <p style={{ margin: "3px 0 0", fontSize: "1.2rem", fontWeight: 800, fontFamily: "monospace", color: "#38bdf8", letterSpacing: "0.06em" }}>
                          {item.kode_aset}
                        </p>
                      </div>
                    ...
                    <InfoRow icon="bi-geo-alt"        label="Lokasi"       value={item.lokasi_terakhir} />
                    ...
                    {item.keluhan && (
                      <div style={{ padding: "0.7rem 0", borderBottom: "1px solid #111c2d" }}>
                        ...
                        <p style={{ margin: 0, color: "#94a3b8", fontSize: "0.875rem", lineHeight: 1.6 }}>{item.keluhan}</p>
                      </div>
                    )}
              ...
              <div style={{ padding: "0 1.75rem 1.75rem" }}>
                <SectionDivider icon="bi-arrow-repeat" label="Update Status" />
                ...
                <form onSubmit={handleSubmit} autoComplete="off">
                  ...
                  <div style={{ marginBottom: "1.25rem" }}>
                    ...
                    {status && (
                      <div style={{
                        ...
                        padding: "0.75rem 1rem", borderRadius: "10px", marginTop: "0.75rem",
                        background: selectedMeta.color + "0d",
                        border: `1px solid ${selectedMeta.color}30`,
                      }}>
                        <div style={{ width: 8, height: 8, borderRadius: "50%", background: selectedMeta.color, boxShadow: `0 0 6px ${selectedMeta.color}`, flexShrink: 0 }} />
                        <span style={{ fontSize: "0.82rem", color: selectedMeta.color, fontWeight: 700 }}>{selectedMeta.label}</span>
                        <span style={{ fontSize: "0.78rem", color: "#475569", marginLeft: "auto" }}>Status yang akan disimpan</span>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Lokasi */}
                  <div style={{ marginBottom: "1.25rem" }}>
                    <label className="field-label" style={{ color: lokFocus ? "#38bdf8" : "#64748b" }}>
                      <i className="bi bi-geo-alt"></i>Lokasi Terakhir
                    </label>
                    <input
                      type="text"
                      className="text-input"
                      placeholder="Contoh: Meja Teknisi 1, Ruang Service, Vendor…"
                      value={lokasi}
                  ...
                  <div style={{ marginBottom: "1.75rem" }}>
                    <label className="field-label" style={{ color: catFocus ? "#38bdf8" : "#64748b" }}>
                      <i className="bi bi-chat-left-dots"></i>
                      Catatan
                      <span style={{ marginLeft: "auto", fontSize: "0.7rem", color: "#475569", textTransform: "none", letterSpacing: 0, fontWeight: 400 }}>
                        Opsional — akan dikirim ke pelanggan via email
                      </span>
                    </label>
                    <textarea
                      className="text-input"
                      placeholder="Tulis catatan update, hasil diagnosa, atau info untuk pelanggan…"
                      value={catatan}
